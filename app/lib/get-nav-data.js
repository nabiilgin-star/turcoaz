...
// Node.js DNS çözümleyicisini IPv4 öncelikli çalışmaya zorluyoruz.
if (dns && typeof dns.setDefaultResultOrder === "function") {
  dns.setDefaultResultOrder("ipv4first");
}

// ... üstteki importlar ve dns ayarları aynı kalıyor ...

export const getNavbarData = unstable_cache(
  async () => {
    // ⚡️ BURAYI GÜNCELLEDIK: .eq("show_in_menu", true) filtresini ekledik
    ...
    if (catError) {
      console.log("=== SUPABASE NAVBAR ERROR DETAILS ===");
      console.error("❌ Navbar Categories Fetching Failed! Raw Error:", JSON.stringify(catError, Object.getOwnPropertyNames(catError), 2));
      console.log("=====================================");
...
// ... fonksiyonun ve dosyanın geri kalan tüm kodları tamamen aynı kalacak ...
...
export const getAllSlugsForStaticGeneration = unstable_cache(
  async () => {
    ...
    const allPaths = [...categoryPaths, ...subcategoryPaths, ...productPaths];
    allPaths.categorySlugs = categoryPaths; 
...
export const getFaqs = unstable_cache(
  async () => {
    try {
      ...
      if (error) {
        console.error("❌ FAQ Fetching Failed! Raw Error:", JSON.stringify(error, Object.getOwnPropertyNames(error), 2));
        return [];
      }
      return data || [];
    } catch (catchError) {
      console.error("❌ Unexpected FAQ System Error:", JSON.stringify(catchError, Object.getOwnPropertyNames(catchError), 2));
      return [];
    }
  },
  ["faqs-list"],
...
export const getHeroSettings = unstable_cache(
  async () => {
    try {
      ...
      if (error) {
        console.error("❌ Hero Settings Fetching Failed! Raw Error:", JSON.stringify(error, Object.getOwnPropertyNames(error), 2));
        return [];
      }
      return data || [];
    } catch (catchError) {
      console.error("❌ Unexpected Hero Settings Error:", JSON.stringify(catchError, Object.getOwnPropertyNames(catchError), 2));
      return [];
    }
  },
  ["hero-settings-list"],
